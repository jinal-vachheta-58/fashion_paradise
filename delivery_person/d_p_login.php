...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["login"])) {
        ...
        if (empty($name_err) && empty($email_err) && empty($password_err)) {
            ...
            $sql = "SELECT * FROM delivery_person WHERE username = '$username' AND email = '$email'";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                // Verify password
                // if (password_verify($password, $row['d_password'])) {
                    if($password == $row['d_password']){
                    // start a new session
                    $_SESSION['delivery_person_of_fashion_paradise'] = $username;
                    $_SESSION['delivery_person_id'] = $row['dp_id'];
                ...
                echo "<script>alert('user not found.invaliid username or password');  </script>";
            }
        }
    }
}
