...
if (($_SERVER["REQUEST_METHOD"]) == "POST") {
    if (isset($_POST["signup"])) {
        ...
        $sql_check = "SELECT * FROM customer WHERE username = '$username' and email = '$email'";
        $result_check = mysqli_query($con, $sql_check);
        if (mysqli_num_rows($result_check) > 0) {
            echo "<script>alert('User with the same email or username already exists. Please login.');</script>";
        ...
        if ($name_f == 1 && $pass_f == 1 && $email_f == 1 && empty($password_err) && empty($name_err) && empty($email_err) && isset($username) && isset($email) && isset($password)) {
            ...
            $sql = "INSERT INTO customer (username, email, password) VALUES ('$username', '$email', '$hash_password')";
            ...
            if (!$result) {
                ...
                // echo "<script>alert('Welcome $username $cust_id');</script>";
